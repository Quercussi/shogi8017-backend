...
import com.shogi8017.app.exceptions.{CannotPromote, IllegalMove}
...
class GoldTest extends AnyFunSuite:
  ...
  test("A Gold should move like a Gold") {
    ...
    testSeqBlack.foreach(pos => testAction(BLACK_PLAYER, MoveAction(Position(8, 8), pos), Gold(BLACK_PLAYER), s0))

    val testSeqWhite = generateReachablePositions(WHITE_PLAYER)(Position(2, 2))
    testSeqWhite.foreach(pos => testAction(WHITE_PLAYER, MoveAction(Position(2, 2), pos), Gold(WHITE_PLAYER), s1))
  }
  ...
  test("A Gold should capture like a Gold") {
    ...
    testSeqBlack.foreach(pos => {
      val s0_temp = s0.copy(piecesMap = s0.piecesMap + (pos -> PromotedBishop(WHITE_PLAYER)))
      val r1 = testAction(BLACK_PLAYER, MoveAction(Position(8, 8), pos), Gold(BLACK_PLAYER), s0_temp)
    ...
    testSeqWhite.foreach(pos => {
      ...
      val r0 = testAction(WHITE_PLAYER, MoveAction(Position(2, 2), pos), Gold(WHITE_PLAYER), s1_temp)
...
//  }
  ...
  test("Gold should be able to drop any unoccupied position") {
    ...
    allDroppablePosition.foreach(pos => {
      val r1 = testAction(BLACK_PLAYER, DropAction(pos, GOLD), Gold(BLACK_PLAYER), s0)
    ...
    allDroppablePosition.foreach(pos => {
      val r0 = testAction(WHITE_PLAYER, DropAction(pos, GOLD), Gold(WHITE_PLAYER), s1)
