...
class PromotedSilverTest extends AnyFunSuite:
  ...
  test("A PromotedSilver should move like a PromotedSilver") {
    ...
    testSeqWhite.foreach(pos => testMove(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedSilver(WHITE_PLAYER), s0))

    val testSeqBlack = generateReachablePositions(BLACK_PLAYER)(Position(8, 8))
    testSeqBlack.foreach(pos => testMove(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedSilver(BLACK_PLAYER), s1))
  }
  ...
  test("A PromotedSilver should capture like a PromotedSilver") {
    ...
    testSeqWhite.foreach(pos => {
      ...
      val r0 = testMove(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedSilver(WHITE_PLAYER), s0_temp)
    ...
    testSeqBlack.foreach(pos => {
      ...
      val r1 = testMove(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedSilver(BLACK_PLAYER), s1_temp)
