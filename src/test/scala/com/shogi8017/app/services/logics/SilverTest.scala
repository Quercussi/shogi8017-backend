...
class SilverTest extends AnyFunSuite:
  ...
  test("A Silver should move like a Silver") {
    ...
    testSeqWhite.foreach(pos => testMove(WHITE_PLAYER, MoveAction(Position(2, 2), pos), Silver(WHITE_PLAYER), s0))

    val testSeqBlack = generateReachablePositions(BLACK_PLAYER)(Position(8, 8))
    testSeqBlack.foreach(pos => testMove(BLACK_PLAYER, MoveAction(Position(8, 8), pos), Silver(BLACK_PLAYER), s1))
  }
  ...
  test("Silver should be able to promote when reaching or leaving the last three ranks") {
    ...
    (1 to 3).foreach(col =>
      testMove(WHITE_PLAYER, MoveAction(Position(2, 6), Position(col, 7), false), Silver(WHITE_PLAYER), s0)
    )
    (1 to 3).foreach(col => testMove(WHITE_PLAYER, MoveAction(Position(2, 6), Position(col, 7), true),  PromotedSilver(WHITE_PLAYER), s0))

    Seq(3,5).foreach(col => testMove(WHITE_PLAYER, MoveAction(Position(4, 7), Position(col, 6), false), Silver(WHITE_PLAYER), s0))
    Seq(3,5).foreach(col => testMove(WHITE_PLAYER, MoveAction(Position(4, 7), Position(col, 6), true),  PromotedSilver(WHITE_PLAYER), s0))

    (5 to 7).foreach(col => testMove(BLACK_PLAYER, MoveAction(Position(6, 4), Position(col, 3), false), Silver(BLACK_PLAYER), s1))
    (5 to 7).foreach(col => testMove(BLACK_PLAYER, MoveAction(Position(6, 4), Position(col, 3), true),  PromotedSilver(BLACK_PLAYER), s1))

    Seq(7,9).foreach(col => testMove(BLACK_PLAYER, MoveAction(Position(8, 3), Position(col, 4), false), Silver(BLACK_PLAYER), s1))
    Seq(7,9).foreach(col => testMove(BLACK_PLAYER, MoveAction(Position(8, 3), Position(col, 4), true),  PromotedSilver(BLACK_PLAYER), s1))
  }
  ...
  test("Silver cannot promote outside the last three rank") {
    ...
    generateReachablePositions(WHITE_PLAYER)(Position(2, 4)).foreach(pos =>
      testMoveError(WHITE_PLAYER, MoveAction(Position(2, 4), pos, true), IncorrectPromotionScenario, s0)
    )

    generateReachablePositions(BLACK_PLAYER)(Position(8, 6)).foreach(pos =>
      testMoveError(BLACK_PLAYER, MoveAction(Position(8, 6), pos, true), IncorrectPromotionScenario, s1)
    )
  }
