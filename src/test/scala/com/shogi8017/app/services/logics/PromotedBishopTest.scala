...
class PromotedBishopTest extends AnyFunSuite:
  ...
  test("A PromotedBishop should move in ranged diagonal or unit cross") {
    ...
    testSeqBlack.foreach(pos => testAction(BLACK_PLAYER, MoveAction(Position(4, 6), pos), PromotedBishop(BLACK_PLAYER), s0))

    val testSeqWhite = generateReachablePositions(Position(4, 4))
    testSeqWhite.foreach(pos => testAction(WHITE_PLAYER, MoveAction(Position(4, 4), pos), PromotedBishop(WHITE_PLAYER), s1))
  }
  ...
  test("A PromotedBishop should capture in ranged diagonal or unit cross") {
    ...
    testSeqBlack.foreach(pos => {
      ...
      val r0 = testAction(BLACK_PLAYER, MoveAction(Position(6, 7), pos), PromotedBishop(BLACK_PLAYER), s0_temp)
    ...
    testSeqWhite.foreach(pos => {
      val s1_temp = s1.copy(piecesMap = s1.piecesMap + (pos -> PromotedBishop(BLACK_PLAYER)))
      val r1 = testAction(WHITE_PLAYER, MoveAction(Position(4, 4), pos), PromotedBishop(WHITE_PLAYER), s1_temp)
  ...
  test("A PromotedBishop should not capture a piece of its own side") {
    ...
    testSeqBlack.foreach(pos => {
      ...
      testActionError(BLACK_PLAYER, MoveAction(Position(4, 6), pos), IllegalMove, s0_temp)
    })
  ...
  test("A PromotedBishop should be able to promote") {
    ...
    generateReachablePositions(Position(8, 3)).foreach(pos =>
      testActionError(BLACK_PLAYER, MoveAction(Position(8, 3), pos, true), CannotPromote, s0)
    )
    ...
    generateReachablePositions(Position(2, 7)).foreach(pos =>
      testActionError(WHITE_PLAYER, MoveAction(Position(2, 7), pos, true), CannotPromote, s1)
    )
  }
  ...
  test("Promoted Bishop should not be able to be dropped") {
    ...
    allDroppablePosition.foreach(pos => {
      testActionError(BLACK_PLAYER, DropAction(pos, P_BISHOP), InvalidDropPiece, s0)
    })
