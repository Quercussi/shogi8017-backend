...
class PromotedBishopTest extends AnyFunSuite:
  ...
  test("A PromotedBishop should move in ranged diagonal or unit cross") {
    ...
    testSeqWhite.foreach(pos => testMove(WHITE_PLAYER, MoveAction(Position(4, 4), pos), PromotedBishop(WHITE_PLAYER), s0))

    val testSeqBlack = generateReachablePositions(Position(4, 6))
    testSeqBlack.foreach(pos => testMove(BLACK_PLAYER, MoveAction(Position(4, 6), pos), PromotedBishop(BLACK_PLAYER), s1))
  }
  ...
  test("A PromotedBishop should capture in ranged diagonal or unit cross") {
    ...
    testSeqWhite.foreach(pos => {
      ...
      val r0 = testMove(WHITE_PLAYER, MoveAction(Position(4, 4), pos), PromotedBishop(WHITE_PLAYER), s0_temp)
    ...
    testSeqBlack.foreach(pos => {
      ...
      val r1 = testMove(BLACK_PLAYER, MoveAction(Position(6, 7), pos), PromotedBishop(BLACK_PLAYER), s1_temp)
  ...
  test("A PromotedBishop should not capture a piece of its own side") {
    ...
    testSeqBlack.foreach(pos => {
      ...
      testMoveError(BLACK_PLAYER, MoveAction(Position(4, 6), pos), IllegalMove, s1_temp)
    })
  }
  ...
  test("A PromotedBishop should be able to promote") {
    ...
    generateReachablePositions(Position(2, 7)).foreach(pos =>
      testMoveError(WHITE_PLAYER, MoveAction(Position(2, 7), pos, true), CannotPromote, s0)
    )
    ...
    generateReachablePositions(Position(8, 3)).foreach(pos =>
      testMoveError(BLACK_PLAYER, MoveAction(Position(8, 3), pos, true), CannotPromote, s1)
    )
  }