...
class PromotedPawnTest extends AnyFunSuite:
  ...
  test("A PromotedPawn should move like a PromotedPawn") {
    val s0 = Board.emptyBoard.copy(
      piecesMap = Board.emptyBoard.piecesMap ++ Map(
        Position(2, 2) -> PromotedPawn(WHITE_PLAYER),
        Position(8, 8) -> PromotedPawn(BLACK_PLAYER)
      )
    )
    val s1 = s0.copy(lastAction = Some(Action(WHITE_PLAYER)))

    val testSeqWhite = generateReachablePositions(WHITE_PLAYER)(Position(2, 2))
    testSeqWhite.foreach(pos => testAction(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedPawn(WHITE_PLAYER), s0))

    val testSeqBlack = generateReachablePositions(BLACK_PLAYER)(Position(8, 8))
    testSeqBlack.foreach(pos => testAction(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedPawn(BLACK_PLAYER), s1))
  }

  test("A PromotedPawn should not move like something else") {
    val s0 = Board.emptyBoard.copy(
      piecesMap = Board.emptyBoard.piecesMap ++ Map(
        Position(3, 3) -> PromotedPawn(WHITE_PLAYER),
        Position(7, 7) -> PromotedPawn(BLACK_PLAYER)
      )
    )
    val s1 = s0.copy(lastAction = Some(Action(WHITE_PLAYER)))
  ...
  test("A PromotedPawn should capture like a PromotedPawn") {
    val s0 = Board.emptyBoard.copy(
      piecesMap = Board.emptyBoard.piecesMap ++ Map(
        Position(2, 2) -> PromotedPawn(WHITE_PLAYER),
        Position(8, 8) -> PromotedPawn(BLACK_PLAYER)
    ...
    testSeqWhite.foreach(pos => {
      val s0_temp = s0.copy(piecesMap = s0.piecesMap + (pos -> PromotedPawn(BLACK_PLAYER)))
      val r0 = testAction(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedPawn(WHITE_PLAYER), s0_temp)
      assert(r0.piecesMap.size == 4)
      assert(r0.hands.get(WHITE_PLAYER).contains(Multiset(PAWN)))
      assert(r0.hands.get(BLACK_PLAYER).contains(Multiset.empty))
    })

    val testSeqBlack = generateReachablePositions(BLACK_PLAYER)(Position(8, 8))
    testSeqBlack.foreach(pos => {
      val s1_temp = s1.copy(piecesMap = s1.piecesMap + (pos -> PromotedRook(WHITE_PLAYER)))
      val r1 = testAction(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedPawn(BLACK_PLAYER), s1_temp)
  ...
  test("A PromotedPawn should not capture a piece of its own side") {
    val s0 = Board.emptyBoard.copy(
      piecesMap = Board.emptyBoard.piecesMap ++ Map(
        Position(2, 2) -> PromotedPawn(WHITE_PLAYER),
        Position(8, 8) -> PromotedPawn(BLACK_PLAYER)
  ...
  test("A PromotedPawn should not capture like something else") {
    val s0 = Board.emptyBoard.copy(
      piecesMap = Board.emptyBoard.piecesMap ++ Map(
        Position(3, 3) -> PromotedPawn(WHITE_PLAYER),
        Position(7, 7) -> PromotedPawn(BLACK_PLAYER)
  ...
  test("PromotedPawn should not be able to promote") {
    val s0 = Board.emptyBoard.copy(
      piecesMap = Board.emptyBoard.piecesMap ++ Map(
        Position(2, 6) -> PromotedPawn(WHITE_PLAYER),
        Position(8, 4) -> PromotedPawn(BLACK_PLAYER)
      )
    )
    val s1 = s0.copy(lastAction = Some(Action(WHITE_PLAYER)))
...
//  test("PromotedPawn must be able to return `getAllPossibleMoves` correctly") {
//    val s0 = Board.emptyBoard.copy(
//      piecesMap = Board.emptyBoard.piecesMap ++ Map(
//        Position(1, 1) -> PromotedPawn(WHITE_PLAYER),
//        Position(2, 8) -> PromotedPawn(WHITE_PLAYER),
//        Position(3, 9) -> PromotedPawn(BLACK_PLAYER),
//        Position(4, 2) -> PromotedPawn(BLACK_PLAYER)
//      )
//    )
//    val s1 = s0.copy(lastAction = Some(Action(WHITE_PLAYER)))
//    
...
//      val PromotedPawn = PromotedPawn(WHITE_PLAYER)
//      val tempBoard = s0.copy(piecesMap = s0.piecesMap + (pos -> PromotedPawn))
//      assert(PromotedPawn.getAllPossibleMoves(tempBoard, pos) == allPositions)
//    })
//  }

  test("Promoted Pawn should not be able to be dropped") {
    val s0 = Board.emptyBoard.copy(
      hands = Map(
        WHITE_PLAYER -> Multiset(P_PAWN),
        BLACK_PLAYER -> Multiset(P_PAWN)
      )
    )
    val s1 = s0.copy(lastAction = Some(Action(WHITE_PLAYER)))
    ...
    allDroppablePosition.foreach(pos => {
      testActionError(WHITE_PLAYER, DropAction(pos, P_PAWN), InvalidDropPiece, s0)
    })

    allDroppablePosition.foreach(pos => {
      testActionError(BLACK_PLAYER, DropAction(pos, P_PAWN), InvalidDropPiece, s1)
    })
  }