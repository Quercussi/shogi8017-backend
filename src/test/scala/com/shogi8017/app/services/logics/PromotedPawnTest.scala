...
class PromotedPawnTest extends AnyFunSuite:
  ...
  test("A PromotedPawn should move like a PromotedPawn") {
    ...
    testSeqBlack.foreach(pos => testAction(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedPawn(BLACK_PLAYER), s0))

    val testSeqWhite = generateReachablePositions(WHITE_PLAYER)(Position(2, 2))
    testSeqWhite.foreach(pos => testAction(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedPawn(WHITE_PLAYER), s1))
  }
  ...
  test("A PromotedPawn should capture like a PromotedPawn") {
    ...
    testSeqBlack.foreach(pos => {
      ...
      val r0 = testAction(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedPawn(BLACK_PLAYER), s0_temp)
    ...
    testSeqWhite.foreach(pos => {
      val s1_temp = s1.copy(piecesMap = s1.piecesMap + (pos -> PromotedPawn(BLACK_PLAYER)))
      val r1 = testAction(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedPawn(WHITE_PLAYER), s1_temp)
      assert(r1.piecesMap.size == 4)
      assert(r1.hands.get(WHITE_PLAYER).contains(Multiset(PAWN)))
...
//  }
  ...
  test("Promoted Pawn should not be able to be dropped") {
    ...
    allDroppablePosition.foreach(pos => {
      testActionError(BLACK_PLAYER, DropAction(pos, P_PAWN), InvalidDropPiece, s0)
    })
