...
class PromotedRookTest extends AnyFunSuite:
  ...
  test("A PromotedRook should move in ranged diagonal or unit cross") {
    ...
    testSeqWhite.foreach(pos => testMove(WHITE_PLAYER, MoveAction(whiteRookPos, pos), PromotedRook(WHITE_PLAYER), s0))

    val testSeqBlack = generateReachablePositions(blackRookPos)
    testSeqBlack.foreach(pos => testMove(BLACK_PLAYER, MoveAction(blackRookPos, pos), PromotedRook(BLACK_PLAYER), s1))
  }
  ...
  test("A PromotedRook should not move like something else") {
    ...
    testMoveError(BLACK_PLAYER, MoveAction(Position(3, 5), testSeqBlack.head), IllegalMove, s1)
  }
  ...
  test("A PromotedRook should not be able to jump") {
    ...
    testMoveError(WHITE_PLAYER, MoveAction(Position(4, 4), Position(8, 4)), IllegalMove, s0)
    testMoveError(BLACK_PLAYER, MoveAction(Position(7, 4), Position(2, 4)), IllegalMove, s1)
  }
  ...
  test("A PromotedRook should capture in ranged diagonal or unit cross") {
    ...
    testSeqWhite.foreach(pos => {
      val s0_temp = s0.copy(piecesMap = s0.piecesMap + (pos -> PromotedRook(BLACK_PLAYER)))
      val r0 = testMove(WHITE_PLAYER, MoveAction(Position(4, 4), pos), PromotedRook(WHITE_PLAYER), s0_temp)
    ...
    testSeqBlack.foreach(pos => {
      ...
      val r1 = testMove(BLACK_PLAYER, MoveAction(Position(6, 7), pos), PromotedRook(BLACK_PLAYER), s1_temp)
  ...
  test("A PromotedRook should not capture a piece of its own side") {
    ...
    testSeqWhite.foreach(pos => {
      ...
      testMoveError(WHITE_PLAYER, MoveAction(Position(4, 4), pos), IllegalMove, s0_temp)
    })
    ...
    testSeqBlack.foreach(pos => {
      ...
      testMoveError(BLACK_PLAYER, MoveAction(Position(3, 5), pos), IllegalMove, s1_temp)
    })
  }
  ...
  test("A PromotedRook should not be able to promote") {
    ...
    generateReachablePositions(Position(2, 5)).filter(_.y >= 7).foreach(pos =>
      testMoveError(WHITE_PLAYER, MoveAction(Position(2, 5), pos, true), CannotPromote, s0)
    )

    generateReachablePositions(Position(3, 7)).foreach(pos =>
      testMoveError(WHITE_PLAYER, MoveAction(Position(3, 7), pos, true), CannotPromote, s0)
    )

    generateReachablePositions(Position(8, 5)).filter(_.y <= 3).foreach(pos =>
      testMoveError(BLACK_PLAYER, MoveAction(Position(8, 5), pos, true), CannotPromote, s1)
    )

    generateReachablePositions(Position(7, 3)).foreach(pos =>
      testMoveError(BLACK_PLAYER, MoveAction(Position(7, 3), pos, true), CannotPromote, s1)
    )
  }