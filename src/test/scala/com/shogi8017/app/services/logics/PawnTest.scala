...
import com.shogi8017.app.errors.{ExpectingPromotion, IllegalDrop, IllegalMove, IncorrectPromotionScenario}
...
class PawnTest extends AnyFunSuite:
  test("Pawn should move forward by one square") {
    val s1 = testAction(WHITE_PLAYER, MoveAction(Position(2, 3), Position(2, 4)), Pawn(WHITE_PLAYER))
    testAction(BLACK_PLAYER, MoveAction(Position(2, 7), Position(2, 6)), Pawn(BLACK_PLAYER), s1)
  }
  ...
  test("Pawn should not move backward") {
    ...
    testActionError(WHITE_PLAYER, MoveAction(Position(4, 4), Position(4, 3)), IllegalMove, newBoard1)

    val newBoard2 = Board(piecesMap = newPieces, lastAction = Some(Action(WHITE_PLAYER)))
    testActionError(BLACK_PLAYER, MoveAction(Position(6, 6), Position(4, 7)), IllegalMove, newBoard2)
  }
  ...
  test("Pawn should not move like something else") {
    ...
    val newBoard1 = Board(newPieces)
    testActionError(WHITE_PLAYER, MoveAction(Position(4, 4), Position(5, 6)), IllegalMove, newBoard1)
  ...
  test("Pawn should not capture a piece of its own side") {
    ...
    val newBoard1 = Board(newPieces)
    testActionError(WHITE_PLAYER, MoveAction(Position(4, 4), Position(4, 5)), IllegalMove, newBoard1)

    val newBoard2 = Board(piecesMap = newPieces, lastAction = Some(Action(WHITE_PLAYER)))
    testActionError(BLACK_PLAYER, MoveAction(Position(5, 5), Position(5, 4)), IllegalMove, newBoard2)
  }
  ...
  test("Pawn should not capture backward") {
    ...
    val newBoard1 = Board(newPieces)
    testActionError(WHITE_PLAYER, MoveAction(Position(4, 6), Position(3, 5)), IllegalMove, newBoard1)

    val newBoard2 = Board(piecesMap = newPieces, lastAction = Some(Action(WHITE_PLAYER)))
    testActionError(BLACK_PLAYER, MoveAction(Position(3, 5), Position(4, 6)), IllegalMove, newBoard2)
  }
  ...
  test("Pawn should promote when reaching the last rank") {
    ...
    testActionError(WHITE_PLAYER, MoveAction(Position(1, 8), Position(1, 9), false), ExpectingPromotion, board0)
    val board1 = testAction(WHITE_PLAYER, MoveAction(Position(1, 8), Position(1, 9), true), PromotedPawn(WHITE_PLAYER), board0)

    testActionError(BLACK_PLAYER, MoveAction(Position(1, 2), Position(1, 1), false), ExpectingPromotion, board1)
    testAction(BLACK_PLAYER, MoveAction(Position(1, 2), Position(1, 1), true), PromotedPawn(BLACK_PLAYER), board1)
  }
  ...
  test("Pawn cannot promote outside the last three rank") {
    ...
    testActionError(WHITE_PLAYER, MoveAction(Position(1, 5), Position(1, 6), true), IncorrectPromotionScenario, board0)
    testActionError(BLACK_PLAYER, MoveAction(Position(2, 5), Position(2, 4), true), IncorrectPromotionScenario, board1)
  }
  ...
  test("Pawn should be able to drop on the last rank") {
    ...
    allUndroppablePosition(WHITE_PLAYER).foreach(pos => {
      testActionError(WHITE_PLAYER, DropAction(pos, PAWN), IllegalDrop, s0)
    })

    allUndroppablePosition(BLACK_PLAYER).foreach(pos => {
      testActionError(BLACK_PLAYER, DropAction(pos, PAWN), IllegalDrop, s1)
    })
  }
  ...
  test("Pawn cannot drop such that it ends in a checkmate") {
    ...
    testActionError(WHITE_PLAYER, DropAction(Position(5, 7), PAWN), IllegalDrop, s0)
    testActionError(BLACK_PLAYER, DropAction(Position(5, 3), PAWN), IllegalDrop, s1)
  }
  ...
  test("Pawn cannot drop on a file already occupied by another pawn of the same player") {
    ...
    testActionError(WHITE_PLAYER, DropAction(Position(3, 4), PAWN), IllegalDrop, s0)
    testActionError(BLACK_PLAYER, DropAction(Position(4, 5), PAWN), IllegalDrop, s1)
  }