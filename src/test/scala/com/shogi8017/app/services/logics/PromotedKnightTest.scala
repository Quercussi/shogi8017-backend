...
class PromotedKnightTest extends AnyFunSuite:
  ...
  test("A PromotedKnight should move like a PromotedKnight") {
    ...
    testSeqWhite.foreach(pos => testMove(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedKnight(WHITE_PLAYER), s0))

    val testSeqBlack = generateReachablePositions(BLACK_PLAYER)(Position(8, 8))
    testSeqBlack.foreach(pos => testMove(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedKnight(BLACK_PLAYER), s1))
  }
  ...
  test("A PromotedKnight should capture like a PromotedKnight") {
    ...
    testSeqWhite.foreach(pos => {
      ...
      val r0 = testMove(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedKnight(WHITE_PLAYER), s0_temp)
    ...
    testSeqBlack.foreach(pos => {
      ...
      val r1 = testMove(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedKnight(BLACK_PLAYER), s1_temp)
