...
class PromotedKnightTest extends AnyFunSuite:
  ...
  test("A PromotedKnight should move like a PromotedKnight") {
    ...
    testSeqWhite.foreach(pos => testAction(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedKnight(WHITE_PLAYER), s0))

    val testSeqBlack = generateReachablePositions(BLACK_PLAYER)(Position(8, 8))
    testSeqBlack.foreach(pos => testAction(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedKnight(BLACK_PLAYER), s1))
  }
  ...
  test("A PromotedKnight should capture like a PromotedKnight") {
    ...
    testSeqWhite.foreach(pos => {
      val s0_temp = s0.copy(piecesMap = s0.piecesMap + (pos -> PromotedKnight(BLACK_PLAYER)))
      val r0 = testAction(WHITE_PLAYER, MoveAction(Position(2, 2), pos), PromotedKnight(WHITE_PLAYER), s0_temp)
    ...
    testSeqBlack.foreach(pos => {
      ...
      val r1 = testAction(BLACK_PLAYER, MoveAction(Position(8, 8), pos), PromotedKnight(BLACK_PLAYER), s1_temp)
...
//  }
  ...
  test("Promoted Knight should not be able to be dropped") {
    ...
    allDroppablePosition.foreach(pos => {
      testActionError(WHITE_PLAYER, DropAction(pos, P_KNIGHT), InvalidDropPiece, s0)
    })

    allDroppablePosition.foreach(pos => {
      testActionError(BLACK_PLAYER, DropAction(pos, P_KNIGHT), InvalidDropPiece, s1)
    })
  }